...
ps <- phyloseq(otu_table(otu, taxa_are_rows=FALSE), 
               sample_data(samples), 
               tax_table(taxon))
ps #411 taxa and 59 samples
cnat<-subset_samples(ps, Species=="CNAT")
...
ps #411 taxa and 59 samples
mcav<-subset_samples(ps, Species=="MCAV")
...
pdf("DistanceToCentroid.pdf",bg ="white",width=8.5)
p2<-ggplot(dis.treat,aes(x=Species,y=distance))+
  geom_boxplot()+
...
```{r, echo=FALSE}
ps2_ra<-transform_sample_counts(ps2, function(OTU) OTU/sum(OTU))
vibrio<-subset_taxa(ps2_ra, Genus=="Vibrio")
vibrio #8 taxa and 59 samples
otu.vib = as(otu_table(vibrio), "matrix")
otu.vib<-as.data.frame(otu.vib)
otu.vib<-rownames_to_column(otu.vib,var="Sample")
#export ASV sequences for supplemental table
write.table(otu.vib,"Vibrio_ASVs.txt",sep="\t",col.names=NA)
names(otu.vib)[2]<-"ASV1"
names(otu.vib)[3]<-"ASV2"
names(otu.vib)[4]<-"ASV3"
names(otu.vib)[5]<-"ASV4"
names(otu.vib)[6]<-"ASV5"
names(otu.vib)[7]<-"ASV6"
names(otu.vib)[8]<-"ASV7"
names(otu.vib)[9]<-"ASV8"
meta.vib = as(sample_data(vibrio), "matrix")
meta.vib<-as.data.frame(meta.vib)
meta.vib<-rownames_to_column(meta.vib,var="Sample")
otu.vib.meta<-merge(meta.vib,otu.vib,"Sample")
otu_long_p<-reshape2::melt(otu.vib.meta,id.vars=c("Sample","Species","Genotype","Fragment","Temp","Time","Description","Date","Timeframe"),variable.name="ASV",value.name="Proportion")

cols<-c("21C"="#0072B2","26C"="#56B4E9","31C"="#E69F00")
cols2<-c("CNAT"="#999999","MCAV"="#000000")

otu_long_p_ASV <- otu_long_p %>% group_by(Temp,ASV) %>% summarize(ASV_mean=mean(Proportion))

pdf("Vibrio_ASVs.pdf",bg = "white",width=8,height=5)
